  ...
  "Statement": [
    {
      ...
      "Principal": {
        ...
        "AWS": "${data.terraform_remote_state.ecs.outputs.cross_account_role_arn}"
      },
  ...
  policy = <<EOF
{
  ...
  "Statement": [
    ...
    {
      "Action": "sts:*",
      "Resource": "${data.terraform_remote_state.ecs.outputs.cross_account_role_arn}",
      "Effect": "Allow"
    }
  ]
}
  ...
  policy = <<EOF
{
  ...
  "Statement": [{
    ...
    "Principal": {
      "AWS": [
        "${data.terraform_remote_state.ecs.outputs.cross_account_role_arn}",
